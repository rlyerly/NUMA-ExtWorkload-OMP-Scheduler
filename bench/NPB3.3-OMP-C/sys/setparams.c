...
void write_numa_info(FILE *fp, char class);
...
enum benchmark_types {SP, BT, LU, MG, FT, IS, EP, CG, UA, DC, NUMA};
...
void get_info(char *argv[], int *typep, char *classp) 
{
  ...
  *classp = *argv[2];
  ...
  if      (!strcmp(argv[1], "sp") || !strcmp(argv[1], "SP")) *typep = SP;
	...
	else if (!strcmp(argv[1], "numa") || !strcmp(argv[1], "NUMA")) *typep = NUMA;
  else {
    printf("setparams: Error: unknown benchmark type %s\n", argv[1]);
    exit(1);
  }

	// NUMA test is class-agnostic
	if(*typep == NUMA) *classp = 'S';
}
...
/*
 * write_numa_info(): Write NUMA specific info to config file
 */

void write_numa_info(FILE *fp, char class)
{
	// Nothing for now, provided for compatibility
	fprintf(fp, "// Nothing for now...\n");
}
